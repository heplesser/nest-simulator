...
main( int argc, char* argv[] )
{
  ...
  nest::kernel().vp_manager.initialize( false );

  nest::kernel().vp_manager.set_num_threads( thr_req );

  // nest::kernel().random_manager.initialize( false );
  nest::kernel().connection_manager.initialize( false );
  nest::kernel().model_manager.initialize( false );

  //  dictionary d = nest::get_kernel_status();
  std::cerr << "Threads after startup: " << nest::kernel().vp_manager.get_num_threads() << std::endl;
