...
function( NEST_RESOLVE_FLAG_OPTION option_value default_value result_var )
  ...
  set( _false_re "^(0|OFF|NO|FALSE|N|IGNORE|NOTFOUND)?$" )
  set( _true_re  "^(1|ON|YES|TRUE|Y)$" )
  string( TOUPPER "${option_value}" _upper )
  if ( _upper MATCHES "${_false_re}" )
    set( ${result_var} "" PARENT_SCOPE )
  elseif ( _upper MATCHES "${_true_re}" )
...
# Claim NESTKERNEL_API_CXX as a typed FILEPATH cache variable so that a
# user-supplied -DNESTKERNEL_API_CXX=... does not linger as UNINITIALIZED
# and the variable appears in cmake-gui / ccmake.
# Must be called before NEST_FIND_PYTHON() so the Cython search is skipped
# when a pre-generated file is supplied.
# Usage:
#   (not set)              — run Cython as normal.
#   -DNESTKERNEL_API_CXX=<file> — use that file, skip Cython.  A warning is
#                           issued at summary time if the file is absent (not
#                           fatal: it may be generated by a preceding build step).
function( NEST_PROCESS_WITH_NESTKERNEL_API_CXX )
...
function( NEST_FIND_PYTHON )
  ...
  # Cython is required only when no pre-generated file is supplied.
  if ( NOT NESTKERNEL_API_CXX )
...
function( NEST_PROCESS_WITH_DETAILED_TIMERS )
  set( TIMER_DETAILED ${with-detailed-timers} PARENT_SCOPE )
endfunction()

function( NEST_PROCESS_WITH_CYCLE_TIMERS )
  if ( with-cycle-timers AND NOT with-detailed-timers )
    message( FATAL_ERROR "To enable cycle timers, you must also enable detailed timers." )
  endif ()
  set( CYCLE_TIMERS ${with-cycle-timers} PARENT_SCOPE )
endfunction()

function( NEST_PROCESS_WITH_THREADED_TIMERS )
  set( THREADED_TIMERS ${with-threaded-timers} PARENT_SCOPE )
endfunction()
